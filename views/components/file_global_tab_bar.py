...
class FileGlobalTabBar(QtWidgets.QWidget):
    triggered = QtCore.Signal(tuple) # tuple of (action, data)
    ...
    def __init__(self, parent=None):
        ...
        self.show2DAction.setData(FILE_PREVIEW_ACTIONS.SHOW_2D)
        ...
        self.show3DAction.setData(FILE_PREVIEW_ACTIONS.SHOW_3D)
        ...
        self.gestureControlsAction.setData(FILE_PREVIEW_ACTIONS.GESTURE_CONTROLS)
    ...
    def __handleToolBarTriggered(self, action: QtGui.QAction):
        if action.data() == FILE_PREVIEW_ACTIONS.SHOW_2D:
            self.show2DAction.setVisible(False)
            self.show3DAction.setVisible(True)
            self.triggered.emit((action.data(), None))

        if action.data() == FILE_PREVIEW_ACTIONS.SHOW_3D:
            self.show3DAction.setVisible(False)
            self.show2DAction.setVisible(True)
            self.triggered.emit((action.data(), None))

        if action.data() == FILE_PREVIEW_ACTIONS.GESTURE_CONTROLS:
